      ...
      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ position: "relative", width: "100%" }}>
          <input value={password} onChange={e => setPassword(e.target.value)}
            placeholder="Password (min. 6 karakter)" type={showPassword ? "text" : "password"}
            style={{ ...inputStyle, paddingRight: 44 }}
            onKeyDown={e => e.key === "Enter" && handle()} />
          <button type="button" onClick={() => setShowPassword(v => !v)} style={{
            position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)",
            background: "none", border: "none", cursor: "pointer", padding: 4,
            color: showPassword ? "#C8A96E" : "#4A5068", fontSize: 16, lineHeight: 1,
...
export default function QuranHafalanApp() {
      ...
      <div style={{ display: "flex", justifyContent: "space-around", padding: "10px 0 6px" }}>
        {[
          ...
          <button key={nav.id} onClick={() => setScreen(nav.id)} style={{
            ...
            color: screen === nav.id ? "#C8A96E" : "#4A5068",
  ...
  const Dashboard = () => (
    <div style={{ padding: "0 20px 140px" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {dueToday.length > 0 ? dueToday.slice(0, 3).map(s => (
          <div key={s.id} style={{
            ...
            <button onClick={() => { setSelectedSurah(s); setScreen("muraja"); }} style={{
              background: "#1E2535", border: "1px solid #2A3045", borderRadius: 8,
      ...
      {/* Quick Start */}
      <button onClick={() => setScreen("hafalan")} style={{
